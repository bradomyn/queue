...
Define_Module(WRSource7);

void WRSource7::initialize()
{
    WRSourceBase7::initialize();
...
void WRSource7::handleMessage(cMessage *msg)
{
    if ((numJobs < 0 || numJobs > jobCounter) )  // && (stopTime < 0 || stopTime > simTime()))
    {
    	...
    	for( it=v.begin(); it!=v.end(); it++ ) {
    		sendDirect(*it, targetModule, "sendDirect");	// WRSwitch7
    		numSent++;
    	}
...
std::vector<Job*> WRSource7::generateJobs(int number) {
	std::vector<Job*> jobs;
...
Define_Module(WRSourceOnce7);

void WRSourceOnce7::initialize()
{
    WRSourceBase7::initialize();
...
void WRSourceOnce7::handleMessage(cMessage *msg)
{
